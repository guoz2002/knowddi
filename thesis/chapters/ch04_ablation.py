...
[GRAY]（5）去噪机制提升少数类表示的内在机制：从图神经网络消息传递的角度理解，去噪机制本质上是在对边的传播权重进行选择性压缩——它通过denoise_head打分识别出与药物对语义相关性低的嘈杂邻居，并将其消息权重压至接近零，从而使每个药物节点在聚合时更集中地从"高相关邻居"吸收表征信息。对于低频DDI类别而言，其正样本药物对在知识图谱中往往缺乏丰富的直接邻居连接，一旦噪声边的干扰被压制，稀疏但高质量的路径信号就能更清晰地传递给下游分类器，从而改善模型对该类别的识别能力。这也从机制层面解释了为什么去噪对整体ACC的提升作用不如对Macro-F1明显：整体ACC主要由高频类别主导，其邻居信息本已丰富，去噪带来的边际增益相对有限。  # 修改新增：补充去噪机制提升少数类的内在传播机制分析
...
[GRAY]（5）多标签图稠密场景下GSL自发受抑的机理解析：从信息论角度理解，知识图谱中两个药物节点之间若已存在多条高权重路径（多跳连通性强），则 GraphSAGE 的多跳聚合本身已能捕获足够丰富的邻域信号，此时再通过补全机制添加新边，等效于在已趋于饱和的信息通道上叠加冗余信号，对最终表示向量的边际改变极小。TWOSIDES数据集中645种药物两两之间的DDI标注密度相对较高，每对药物的正标签平均约为3~4个（多标签），这使得子图提取时每个药物节点的平均有效邻居数显著高于DrugBank，相应地降低了补全机制的实际作用空间。去噪机制在此背景下同样面临类似的困境：由于有效边信号强、噪声边信号相对弱，去噪门控头在高密度子图中难以学到足够清晰的"信号-噪声"边界，导致其重加权结果趋近于均一化（接近原始权重），边际贡献因此收缩为近零。  # 修改新增：补充TWOSIDES多标签稠密图下GSL受抑的信息论机理分析
...
4.5.2 差异化贡献格局的规律性总结  # 修改新增：改为聚焦规律性归纳，有利于答辩阐述

[GRAY]跨越两个数据集的消融实验共同揭示了一个可复现的规律：去噪与补全两类机制对不同评价维度呈现出系统性的差异化贡献格局，而非简单的叠加或抵消关系。

[GRAY]具体而言，去噪机制在DrugBank多分类任务中对Macro-F1（类别均衡精度）呈正向作用，说明它有助于减少传播噪声对少数类别表示的污染；而补全机制在同一数据集中对整体一致性指标（ACC、Cohen's κ）呈正向作用，但对Macro-F1呈负向作用，说明它引入的新边更多服务于高频关系的结构补强，而非少数类别的表示改善。在TWOSIDES多标签任务中，两者的边际效应均趋于中性，进一步说明机制效果受数据分布特性的调节。

[GRAY]这一差异化格局在理论层面具有以下几点深刻意义：

[GRAY]其一，它表明去噪与补全在功能上是互补而非等价的——二者作用于图结构的不同层面（去噪压缩噪声边、补全添加缺失边），其对模型性能的影响路径也截然不同。这一发现从实证角度验证了本文将二者解耦设计而非一体化处理的科学合理性。

[GRAY]其二，这种差异化贡献格局并非偶然，而是与数据集的内在特性密切相关：DrugBank多分类任务中类别分布相对不均衡，稀疏的少数类边更容易受到噪声干扰，因此去噪对Macro-F1的正向效果更加突出；而TWOSIDES多标签任务中关联图相对稠密，噪声影响被分散，使得两种机制的边际效应均趋于中性。这一规律提示研究者在选择图结构优化策略时，应首先审视数据集的图密度与标签分布特征。

[GRAY]其三，补全机制对整体一致性指标（ACC、Cohen's κ）的正向作用与其对类别均衡指标（Macro-F1）的负向作用同时存在，说明补全引入的新边在优先修复"全局结构缺口"的同时，可能对局部少数类表示造成稀释。这种"整体均衡与局部代表性之间的权衡"现象，与图神经网络文献中关于图稠密度与表示质量关系的一般性讨论相呼应，具有一定的理论普遍性。  # 修改新增：扩充4.5.2，删除"答辩"措辞
...
[GRAY]4.5.4 消融设计边界与潜在混淆  # 修改新增：用机制分析替代空泛的局限性列举

[GRAY]本章消融设计围绕"去噪"与"补全"两个二元开关构建，这一设计在逻辑上清晰，但也存在若干值得明确说明的内在边界：

[GRAY]首先，go噪与补全两个门控头虽然参数不共享，但同时依赖共享的上游Encoder进行节点表示，因此两者并非完全解耦——Encoder的优化方向在不同开关组合下可能产生微小差异，进而影响边权重的计算结果。实验观测到两组单变体（Denoise-only与Completion-only）的Encoder权重余弦相似度约为0.86~0.93，说明共享Encoder的影响在可接受范围内，但不能视为绝对为零。

[GRAY]其次，补全机制的实际效果与其激活比例密切相关。在DrugBank上，补全激活比例较高，引入的新边对整体图结构的影响可量化；而在TWOSIDES上，补全比例自发收敛至0.62%~0.79%，此时"补全是否生效"的衡量本身就存在歧义。本文将这类激活不充分的情况纳入机制行为分析（见4.4.2节），而非将其归结为实验缺陷。

[GRAY]第三，本章消融仅覆盖DrugBank与TWOSIDES，对单一DDI类别数目（86 vs. 200）和图稠密度的差异进行了对比分析；但在图结构更异质的场景（如含多类实体节点的知识图谱，或药物数量超过3000的大规模数据集）下，去噪与补全的边际贡献格局是否会发生质变，仍有待后续研究在更多数据集上加以检验。
"""
    },
    "4.6": {
        "title": "典型药物对知识子图路径分析",
